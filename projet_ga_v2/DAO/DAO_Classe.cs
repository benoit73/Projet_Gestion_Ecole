...
namespace projet_ga_v2.DAO
{
    internal class DAO_Classe
    {
        ...
        public void AddEleveToClasse(Classe classe, Eleve eleve)
        {
            using (var context = new Benoit73SymfonyV5Context())
            {
                Classe classe1 = context.Classes.Include("Eleves").FirstOrDefault(c => c.Id == classe.Id);
                Eleve eleve1 = context.Eleves.Include("Classe").FirstOrDefault(e => e.Id == eleve.Id);

                classe1.Eleves.Add(eleve1);
                context.SaveChanges();
            }
        }
